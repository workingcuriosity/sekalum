---
title: Legal
version: 1.0.0
status: Active
category: Project
canonical: true
maintainer: Working Curiosity
contact: luiscyphre404@gmail.com
license: AGPL-3.0-only
copyright: "© 2026 Working Curiosity"
target_audience:
  - Users
  - Contributors
  - Auditors
dependent_documents:
  - LICENSE
  - NOTICE
  - DISCLAIMER.md
  - docs/project/THIRD_PARTY_SOFTWARE.md
change_history:
  - version: 1.0.0
    date: 2026-07-16
    change: Adds required canonical metadata.
---

# Legal Information

## Copyright

© 2026 Working Curiosity

## Maintainer

Working Curiosity

## Contact

luiscyphre404@gmail.com

## License

Credential HUB is licensed under the GNU Affero General Public License v3.0 only (AGPL-3.0-only).

Authoritative legal documents:

- `/LICENSE`
- `/NOTICE`
- `/DISCLAIMER.md`

## Third-party Software

Third-party components remain subject to their respective licenses. The lockfile-based component, version, and declared-license inventory is maintained in [Third-Party Software](THIRD_PARTY_SOFTWARE.md).

## Security Contact

Please report potential security issues responsibly via:

luiscyphre404@gmail.com
